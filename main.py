...
def bot_worker(email):
    ...
    while True:
        ...
        # إذا لم يجد المستخدم (بسبب الحذف) أو كانت الحالة STOPPED يخرج من الحلقة
        if not u or "STOPPED" in str(u.get("status")): 
            break
        ...
        if datetime.now().second == 0:
            ...
            try:
                ...
                if len(prices) >= 90:
                    part1, part2, part3 = prices[0:30], prices[30:60], prices[60:90]
                    ...
                    if side:
                        ...
                        if "buy" in res:
                            ...
                            poc = json.loads(ws.recv()).get("proposal_open_contract", {})
...
def home():
    ...
    <body style='background:#0d1117;color:white;text-align:center;font-family:sans-serif;padding:30px'>
        <h2 style='color:#00e676'>V19 FINAL - AUTO DELETE ON RESET</h2>
        <div id=login_div>
        ...
        <div id=stats style='display:none'>
            ...
            <button onclick="reset()" style="background:#da3633; color:white; border:none; padding:12px 25px; border-radius:8px; cursor:pointer; font-weight:bold">STOP & RESET ALL DATA</button>
        </div>
        <script>
            let email = "";
            let interval = null;
            ...
            function startUI(){
                ...
                interval = setInterval(async()=>{
                    ...
                    if(d.found){
                        ...
                        // إذا تم الحذف من قاعدة البيانات، نوقف التحديث ونعيد الصفحة
            ...
            async function reset(){
                if(!confirm("سيتم حذف كافة البيانات من السجل وإيقاف البوت نهائياً. هل أنت متأكد؟")) return;
...
def reset_bot():
    ...
    # التعديل الجوهري: حذف السجل بالكامل من MongoDB
