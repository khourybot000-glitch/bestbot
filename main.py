...
# --- CONFIGURATION ---
BOT_TOKEN = "8511172742:AAHNk35ROiv4w7YiTdodQwkuU-9sxSxt4Gk"
...
def user_trading_loop(chat_id, token):
    ...
    print(f"🛑 [THREAD END] Closed for: {chat_id}")
...
def handle_outcome(chat_id, token, profit):
    ...
    bot.send_message(chat_id, f"{'✅ WIN' if is_win else '❌ LOSS'} (${profit})\nNet Profit: ${new_profit}")

    if streak >= 2 or new_profit >= session_db["target_profit"]:
        stop_signals[chat_id] = True
        active_sessions_col.delete_one({"chat_id": chat_id})
        bot.send_message(chat_id, "🏁 Session Stopped (TP/SL).", reply_markup=main_keyboard())
    
    trade_locks[chat_id] = False

# --- 🔥 AUTO RECOVERY ENGINE 🔥 ---
def recover_sessions():
    print("🔍 [SYSTEM] Searching MongoDB for active sessions...")
    ...
    for s in active_list:
        ...
        t.start()
        try: bot.send_message(chat_id, "🔄 System Restarted: Your trading session has been automatically recovered!")
        except: pass
...
def start_cmd(m):
    ...
    bot.send_message(m.chat.id, "♻️ Reset. Enter Registered Email:", reply_markup=main_keyboard())
...
def menu_handler(m):
    ...
    active_sessions_col.delete_one({"chat_id": m.chat.id})
    if m.text == 'STOP 🛑':
        bot.send_message(m.chat.id, "🛑 Stopped.", reply_markup=main_keyboard())
    else:
...
def admin():
    ...
    <html><head><title>Admin Panel</title><style>
        ...
        .card{background:#1e293b;padding:30px;border-radius:15px;display:inline-block;width:100%;max-width:600px;box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);}
        input,select{padding:12px;margin:5px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:white;width:80%;}
        button{padding:12px 25px;background:#38bdf8;border:none;border-radius:8px;font-weight:bold;cursor:pointer;margin-top:10px;color:white;}
        table{width:100%;margin-top:30px;border-collapse:collapse;}
        th{background:#334155;padding:12px;text-align:left;}
        td{padding:12px;border-bottom:1px solid #334155;text-align:left;}
...
if __name__ == '__main__':
    ...
    recover_sessions() # استعادة الـ Threads فوراً
