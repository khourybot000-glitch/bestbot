...
# الاستراتيجية: DIGITDIFF (مطلوب حاجز/رقم هدف)
...
def stop_bot(email, clear_data=True, stop_reason="Stopped Manually"):
    ...
    if clear_data:
        ...
        # مسار المضاعفة الفورية (Auto-Retry)
        print(f"⚠ [INFO] WS closed for {email}. Immediate Martingale in progress. Attempting reconnect...")
...
def send_trade_order(email, stake, currency, contract_type_param, barrier_value=None):
    # (لم تتغير، تستخدم barrier_value كمدخل)
...
def check_pnl_limits(email, profit_loss, last_action_type, ws_app):
    """ تحديث الإحصائيات واتخاذ قرار بشأن المضاعفة/الإيقاف/إعادة الاتصال الفوري """
    ...
    if profit_loss > 0:
        # حالة الربح: نعود للرهان الأساسي ونمسح الرقم الهدف المخزن
        ...
        print(f"💸 [MARTINGALE] Lost. Next stake calculated: {new_stake:.2f}. Closing connection to re-enter...")
    ...
    while True:
        ...
        def on_message_wrapper(ws_app, message):
            ...
            if msg_type == 'tick':
                ...
                # 2. تحديد الرقم الهدف (Barrier)
                if is_martingale_retry and current_data['last_trade_barrier'] is not None:
                    # المضاعفة: استخدام الرقم المخزن من الصفقة الخاسرة
                    barrier_value = current_data['last_trade_barrier']
                    print(f"💡 [MARTINGALE] Immediate Retry. Reusing last barrier: {barrier_value}")
                else:
                    # الدخول الأساسي: حساب الرقم الهدف من التيك الحالي
                    ...
                    try:
                        ...
                        print(f"⚠ [DIGIT ERROR] Could not extract last digit from price: {current_price}. Skipping trade.")
                        return
                        
                    # تخزين الرقم الهدف المستخدم في الدخول الأساسي ليُستخدم في المضاعفة
                    current_data['last_trade_barrier'] = barrier_value
                    print(f"🎯 [BASE ENTRY] Time entry condition met (:{current_second}). New Barrier set: {barrier_value}")
        ...
        def on_close_wrapper(ws_app, code, msg):
            print(f"⚠ [PROCESS] WS closed for {email}. Auto-retry logic engaged.")
            is_contract_open[email] = False
        ...
        print(f"💤 [PROCESS] Waiting {RECONNECT_DELAY} second before retrying connection (Martingale: Step {current_data.get('current_step', 0)} / Barrier: {current_data.get('last_trade_barrier', 'N/A')})...")
...
{% if session_data and session_data.is_running or session_data.stop_reason == "Disconnected (Auto-Retry)" %}
    {% set strategy = contract_type + " (1 تيك | مضاعفة فورية x" + 19|string + ")" %}
    
    <p class="status-running">✅ البوت قيد التشغيل! (يتم التحديث تلقائيًا)</p>
    {% if session_data.stop_reason == "Disconnected (Auto-Retry)" %}
    <p style="color:orange; font-weight:bold;">⚠ محاولة إعادة دخول فورية (خطوة مضاعفة {{ session_data.current_step }}/{{ martingale_steps }})</p>
    {% endif %}
...
def start_bot():
    ...
    flash(f'Bot started successfully. Currency: {currency}. Account: {account_type.upper()}. Strategy: {CONTRACT_TYPE} 1 Tick (x19 Immediate Martingale)', 'success')
    return redirect(url_for('index'))
