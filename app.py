...
# ==========================================================
# BOT CONSTANT SETTINGS (R_100 | Dual Entry | Waiting Sec 30)
# ==========================================================
...
LAST_ENTRY_SECOND = {}         # لحفظ آخر ثانية تم الدخول فيها

# ID لطلب التيك التاريخي (لم يعد مستخدماً في هذه الاستراتيجية ولكن نتركه)
TICK_HISTORY_REQUEST_ID = 1000
TICK_COUNT_FOR_ANALYSIS = 0 
...
DEFAULT_SESSION_STATE = {
    ...
    "is_running": False,
    "current_profit": 0.0,
    "current_stake": 1.0,          # رهان موحد لكل صفقة
    ...
    "open_contract_ids": [],               
    "contract_profits": {},                
    "last_barrier_value": str(BARRIER_OFFSET),
}

# --- Persistence functions (UNCHANGED) ---
def load_persistent_sessions():
    # ... (Unchanged persistence functions) ...
...
def calculate_martingale_stake(base_stake, current_step, multiplier):
    """ منطق المضاعفة: ضرب الرهان الأساسي في معامل المضاعفة (4.5) لعدد الخطوات """
    if current_step == 0: 
        return base_stake
    
    # استخدام القوة (Power) لحساب المضاعفة (Base * Multiplier^Step)
    # ملاحظة: يتم مضاعفة قيمة الصفقة الأساسية * 2 (لأن لدينا صفقتين)
...
def send_trade_order(email, stake, currency, contract_type_param, barrier_offset):
    ...
    # الحاجز الآن هو 0.0
    barrier_param = str(barrier_offset) if barrier_offset != 0.0 else "0.0" 
...
def apply_martingale_logic(email):
    """ يطبق منطق المضاعفة بناءً على نتيجة الصفقتين (Higher/Lower) """
    global is_contract_open, MARTINGALE_MULTIPLIER, MARTINGALE_STEPS, MAX_CONSECUTIVE_LOSSES, LAST_ENTRY_SECOND, BARRIER_OFFSET
    current_data = get_session_data(email)
    
    if not current_data.get('is_running'): return

    # يجب أن تكون لدينا نتيجتان فقط (Higher و Lower)
    results = list(current_data['contract_profits'].values())
    
    if len(results) != 2: 
        print("❌ [MARTINGALE ERROR] Incomplete results (Expected 2 results). Resetting stake to base.")
        total_profit = 0
    else:
        total_profit = sum(results) # مجموع أرباح الصفقتين
    ...
    # في حالة الدخول المزدوج، تكون الخسارة عندما تكون النتيجة الصافية سالبة (أي خسارة كلتا الصفقتين أو تعادل واحدة وخسارة الأخرى)
    if total_profit < 0:
        ...
        entry_tag = f"WAITING @ SEC {ALLOWED_ENTRY_SECONDS} (Martingale Step {current_data['current_step']})"
        
        # 🛑 عند الخسارة: نزيل علامة التوقيت وننتظر الثانية 30 التالية للدخول بالمضاعفة
        LAST_ENTRY_SECOND[email] = None 
        
        print(f"🔄 [LOSS] Net PnL: {total_profit:.2f}. Step {current_data['current_step']}. Next Stake (per contract) calculated: {round(new_stake, 2):.2f}. **Ready for Sec {ALLOWED_ENTRY_SECONDS}.**")
        
    # ✅ Win or Draw Condition (ربح صافي أو تعادل)
    else: 
        ...
        # 🛑 عند الربح/التعادل: ننتظر الثانية 30 للدخول بالرهان الأساسي
        LAST_ENTRY_SECOND[email] = None 
        entry_result_tag = "WIN" if total_profit > 0 else "DRAW"
        entry_tag = f"WAITING @ SEC {ALLOWED_ENTRY_SECONDS} (Base Stake)" 
        print(f"✅ [ENTRY RESULT] {entry_result_tag}. Net PnL: {total_profit:.2f}. Stake reset to base: {base_stake_used:.2f}. **Ready for Sec {ALLOWED_ENTRY_SECONDS}.**")
    ...
    is_contract_open[email] = False # إزالة علامة الحظر (مهم للدخول في الثانية 30 التالية)

    currency = current_data.get('currency', 'USD')
    print(f"[LOG {email}] PNL: {currency} {current_data['current_profit']:.2f}, Step: {current_data['current_step']}, Stake/Contract: {current_data['current_stake']:.2f}, Strategy: Dual Entry ({SYMBOL}) | Next Entry: {entry_tag}")
...
def handle_contract_settlement(email, contract_id, profit_loss):
    ...
    # نبدأ منطق المضاعفة فقط بعد إتمام بيع كلتا الصفقتين (الـ Higher والـ Lower)
    if not current_data['open_contract_ids']:
        apply_martingale_logic(email)


def start_new_dual_trade(email):
    """ يرسل صفقتين Higher و Lower بنفس الوقت ونفس القيمة """
    ...
    entry_type_tag = f"BASE ENTRY (Dual H/L)" if current_step == 0 else f"MARTINGALE STEP {current_step} (Dual H/L)"
    
    print(f"🧠 [DUAL ENTRY - {entry_timing_tag}] {entry_type_tag} | Stake/Contract: {round(stake, 2):.2f}. Total Stake: {round(stake*2, 2):.2f}")
    
    # إرسال صفقة Higher
    send_trade_order(email, stake, currency_to_use, CONTRACT_TYPE_HIGHER, BARRIER_OFFSET)
    # إرسال صفقة Lower
    send_trade_order(email, stake, currency_to_use, CONTRACT_TYPE_LOWER, BARRIER_OFFSET)

    # وضع علامة الحظر فوراً (سواء نجح الإرسال أم لا، للتأكد من عدم التكرار)
    is_contract_open[email] = True 
        
    current_data['last_entry_time'] = int(time.time())
    current_data['last_entry_price'] = current_data.get('last_valid_tick_price', 0.0)
    current_data['last_barrier_value'] = str(BARRIER_OFFSET)
    ...
    while True:
        ...
        def on_message_wrapper(ws_app, message):
            ...
            if msg_type == 'tick':
                ...
                # === منطق الدخول (عند الثانية 30 فقط) ===
                if not is_contract_open.get(email):
                    ...
                    if current_second in ALLOWED_ENTRY_SECONDS and LAST_ENTRY_SECOND.get(email) != current_second:
                        ...
                        entry_tag = f"ENTRY @ SEC {current_second}"
                        
                        if current_step == 0:
                            print(f"🧠 [BASE {entry_tag}] Dual Entry (Higher/Lower) initiated.")
                        else:
                            print(f"🧠 [MARTINGALE {current_step} {entry_tag}] Dual Entry (Higher/Lower) initiated.")
                ...
                # في حالة خطأ Buy، نقوم بإلغاء الصفقة الحالية وننتظر الثانية 30 التالية
                if current_data['current_entry_id'] is not None and is_contract_open.get(email):
                    time.sleep(1) 
                    # بما أننا لا نعرف كم صفقة دخلت، الأمان يتطلب إجبار منطق المضاعفة على التنفيذ 
                    # وهذا يتطلب انتظار إغلاق أي عقود مفتوحة أخرى.
                    # للحفاظ على المنطق، سنعيد تفعيل الدخول في الثانية 30 التالية
                    current_data['current_entry_id'] = None
                    is_contract_open[email] = False
                    LAST_ENTRY_SECOND[email] = None 
                    current_data['stop_reason'] = f"API Buy Error: {error_message}"
                    save_session_data(email, current_data)
                    print("🛑 [API ERROR] Buy failed. Waiting for next Sec 30 to re-enter.")
...
{% if session_data and session_data.is_running %}
    {% set strategy = "Dual H/L Entry @ Sec " + allowed_entry_seconds[0]|string + " | Martingale (x" + martingale_multiplier|string + " - Max Steps " + martingale_steps|string + ")" %}
    ...
    <p>Total Stake/Trade: *{{ session_data.currency }} {{ (session_data.current_stake * 2)|round(2) }}*</p>
    ...
    <p style="font-weight: bold; color: #cc6600;">Barrier: {{ session_data.last_barrier_value }}</p>
...
{% else %}
    ...
    <form method="POST" action="{{ url_for('start_bot') }}">
        ...
        <label for="stake">Base Stake (USD/tUSDT) Per Contract:</label><br>
...
def start_bot():
    ...
    flash(f'Bot started successfully. Currency: {currency}. Account: {account_type.upper()}. Strategy: Dual Entry @ Sec {ALLOWED_ENTRY_SECONDS[0]} ({SYMBOL}) with Martingale (x{MARTINGALE_MULTIPLIER} - Max {MARTINGALE_STEPS} Steps, Max {MAX_CONSECUTIVE_LOSSES} Losses)', 'success')
    return redirect(url_for('index'))
