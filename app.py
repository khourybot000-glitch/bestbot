...
def bot_worker(uid):
    ...
    while True:
        ...
        # ✅ فحص الـ Target Profit (TP)
        if u.get("profit", 0) >= float(u.get("tp", 999999)):
            set_status(uid, "TARGET REACHED! STOPPED.")
            break
                ...
                if "history" in res:
                    ...
                    if not pending_direction:
                        if abs(diff) >= 0.7:
                            ...
                            set_status(uid, f"SIGNAL: {current_dir} (WAITING CONFIRM)")
                        else:
                            set_status(uid, "SCANNING (IDLE)...")
                    else:
                        if current_dir == pending_direction:
                            set_status(uid, f"EXECUTING {current_dir}...")
                            ...
                            if "buy" in buy_res:
                                ...
                                if poc.get("is_sold"):
                                    ...
                                    if profit > 0:
                                        ...
                                        set_status(uid, "WIN! CHECKING TP...")
                                    else:
                            ...
                            set_status(uid, "CONFIRM FAILED - RESET")
...
def home():
    ...
    <body style='background:#0d1117;color:white;text-align:center;font-family:sans-serif;padding:20px'>
        <h2 style='color:#00e676'>SMART SNIPER V10 (+TP)</h2>
        <div id=login_div>
            <input id=ev placeholder="Email" style="padding:10px; border-radius:5px"><br><br>
            <button onclick="login()" style="padding:10px 20px; cursor:pointer">ACCESS BOT</button>
        </div>
        
        <div id=settings style='display:none'>
            <input id=tv placeholder="Deriv Token" style="width:280px; padding:10px"><br><br>
            <div style="display:flex; justify-content:center; gap:10px">
                <input id=stk_v value="1.0" type="number" placeholder="Stake" style="width:100px; padding:10px">
                <input id=tp_v value="10.0" type="number" placeholder="Target Profit" style="width:100px; padding:10px">
            </div><br>
            <button onclick="start()" style="background:#00e676; padding:10px 20px; border:none; border-radius:5px; width:280px">START MONITORING</button>
        </div>

        <div id=stats style='display:none'>
            <div style="background:#161b22; padding:20px; border-radius:15px; border:1px solid #30363d">
                <div id=st style='font-size:24px; font-weight:bold; color:#00e676; margin-bottom:10px'>WAITING CYCLE...</div>
                <hr style="border:0.5px solid #30363d">
                <div id=cur_info style='color:#8b949e; font-size:14px; margin-top:10px'></div>
                <div style="font-size:40px; margin:20px 0" id=profit_text>0.00 $</div>
                <div style="color:#8b949e; margin-bottom:15px">Target Profit: <span id=target_text>0</span></div>
            ...
            <br>
            <button onclick="location.reload()" style="background:#ff5252; color:white; border:none; padding:10px 20px; border-radius:5px">STOP & RESET</button>
        </div>
                ...
                setInterval(async()=>{
                    try {
                        ...
                        if(d.found){
                            ...
                            document.getElementById('target_text').innerText = d.tp + " " + (d.currency || "$");
                            document.getElementById('wins_text').innerText = d.wins;
                            document.getElementById('loss_text').innerText = d.losses;
                            document.getElementById('cur_info').innerText = "Symbol: " + d.symbol + " | Acc: " + d.currency;
                            
                            if(d.status.includes("STOPPED") || d.status.includes("REACHED")) document.getElementById('st').style.color = "#ff7b72";
                        }
                    } catch(e) {}
                }, 1000);
            }
