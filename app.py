...
# --- CONFIGURATION ---
BOT_TOKEN = "8309516496:AAF_7c0LJ8tkhNS1S94veQaHuLc2ONlOrPQ"
        ...
        if "history" in res and "prices" in res["history"]:
            ...
            if len(prices) >= 310:
                ...
                if is_m4_up and is_m5_up:
                    target = "PUT"   # التحليل صعود ولكن الدخول هبوط
                elif not is_m4_up and not is_m5_up:
                    target = "CALL"  # التحليل هبوط ولكن الدخول صعود
...
def start_cmd(m):
    ...
    bot.send_message(m.chat.id, "♻️ System Online (Reversed Signals). Enter Email:", reply_markup=main_keyboard())
...
def start_engine(m, token, stake, target):
    ...
    bot.send_message(m.chat.id, "🛰️ **Engine Active (Reversed Mode)**\nWaiting for signal alignment...", reply_markup=main_keyboard(), parse_mode="Markdown")
