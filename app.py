...
def add_log(user_id, message):
    ...
    users_col.update_one({"_id": ObjectId(user_id)}, {"$push": {"logs": {"$each": [log_entry], "$slice": -10}}})

# --- Strategy: 30-Tick Trend ---
    ...
    try:
        ...
        if contract and contract.get("is_sold"):
            ...
            if profit > 0:
                ...
                if losses >= 2:
                    add_log(user_id, "Stop Loss Reached (2 Losses).")
                    users_col.delete_one({"_id": ObjectId(user_id)})
                    return
                users_col.update_one({"_id": ObjectId(user_id)}, {"$set": {"active_contract": None, "total_profit": new_total, "losses": user.get('losses', 0) + 1, "consecutive_losses": losses, "current_stake": round(user['current_stake'] * 19, 2)}})
            
            if new_total >= user.get('take_profit', 9999):
                add_log(user_id, "Target Reached! Shutting down.")
                users_col.delete_one({"_id": ObjectId(user_id)})
    except Exception as e: add_log(user_id, f"Error checking: {str(e)}")
        ...
        try:
            ...
            if time.localtime().tm_sec == 30:
                ...
                if 'history' in res:
                    ...
                    if signal != "NONE":
                        barrier = "-0.6" if signal == "CALL" else "+0.6"
                        ws = websocket.create_connection(DERIV_WS_URL)
                        ws.send(json.dumps({"authorize": user['token']}))
                        if "authorize" in json.loads(ws.recv()):
                            ...
                            if "buy" in trade_res:
                                add_log(user_id, f"Entered {signal} with Barrier {barrier}")
...
<head><style>
    ...
    .log-box{height:120px; overflow-y:auto; color:#8b949e; font-family:monospace}
</style></head>
<body>
    <div class="card">
        <h2 id="title">KHOURY V6.2.2</h2>
        ...
        <div id="stats-ui" style="display:none;">
            <div class="stat-box" id="live-stats">Loading Stats...</div>
            <div class="log-box" id="live-logs">Waiting for logs...</div>
        </div>
    </div>
    <script>
        let timer = null;
        async function update(email) {
            ...
            if(d.found) {
                document.getElementById('live-stats').innerHTML = `<b>Profit:</b> <span style="color:${d.total_profit>=0?'#238636':'#da3633'}">${d.total_profit.toFixed(2)} USD</span><br><b>Wins/Losses:</b> ${d.wins} / ${d.losses}<br><b>Stake:</b> ${d.current_stake}$`;
                document.getElementById('live-logs').innerHTML = d.logs ? d.logs.reverse().join('<br>') : '...';
