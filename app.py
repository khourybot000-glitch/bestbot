...
# ==========================================================
# BOT CONSTANT SETTINGS (R_100 | x2 | انتظار الثانية 0)
# ==========================================================
...
BARRIER_OFFSET = 0.05        # قيمة الإزاحة الرقمية

# الثواني المسموح بها للدخول: عند الثانية 0
ALLOWED_ENTRY_SECONDS = [0] 
LAST_ENTRY_SECOND = {} 

# ID لطلب التيك التاريخي
TICK_HISTORY_REQUEST_ID = 1000
TICK_COUNT_FOR_ANALYSIS = 30 # عدد التيكات المطلوبة للتحليل
...
def stop_bot(email, clear_data=True, stop_reason="Stopped Manually"):
    ...
    with PROCESS_LOCK:
        if email in active_processes:
            ...
            if process.is_alive():
                print(f"🛑 [INFO] Terminating Process for {email} due to {stop_reason}...")
...
def calculate_martingale_stake(base_stake, current_step, multiplier):
    """ منطق المضاعفة: ضرب الرهان الأساسي في معامل المضاعفة (2.0) لعدد الخطوات """
    if current_step == 0: 
        return base_stake
    
    # استخدام القوة (Power) لحساب المضاعفة (Base * Multiplier^Step)
...
def analyze_tick_trend(ticks_list):
    """
    يحلل الاتجاه بمقارنة سعر فتح (أول تيك) وسعر الإغلاق (آخر تيك) لآخر 30 تيك.
    """
    global CONTRACT_TYPE_HIGHER, CONTRACT_TYPE_LOWER, TICK_COUNT_FOR_ANALYSIS

    if len(ticks_list) < TICK_COUNT_FOR_ANALYSIS:
        # هذه الحالة لا يفترض أن تحدث إذا كان طلب الـ history صحيحاً
        print(f"❌ [ANALYSIS] Insufficient ticks ({len(ticks_list)}/{TICK_COUNT_FOR_ANALYSIS}).")
        return None

    # تحويل التيكات إلى أرقام
    prices = [float(p) for p in ticks_list]

    # سعر الفتح هو أول سعر في القائمة
    open_price = prices[0]
    # سعر الإغلاق هو آخر سعر في القائمة
    close_price = prices[-1]

    entry_direction = None

    if close_price > open_price:
        entry_direction = CONTRACT_TYPE_HIGHER  # صاعد
    elif close_price < open_price:
        entry_direction = CONTRACT_TYPE_LOWER   # هابط
    
    print(f"📈 [ANALYSIS] Open: {open_price:.5f}, Close: {close_price:.5f}. Analysed Trend: {entry_direction}")
            
    return entry_direction


def apply_martingale_logic(email):
    """ يطبق منطق المضاعفة بناءً على نتيجة الصفقة الواحدة، ويسمح بالدخول الفوري بعد الخسارة """
    global is_contract_open, MARTINGALE_MULTIPLIER, MARTINGALE_STEPS, MAX_CONSECUTIVE_LOSSES, LAST_ENTRY_SECOND, BARRIER_OFFSET, CONTRACT_TYPE_HIGHER, CONTRACT_TYPE_LOWER
    current_data = get_session_data(email)
    ...
    # بما أنها صفقة واحدة، يجب أن يكون هناك نتيجة واحدة
    if not results or len(results) != 1: 
        print("❌ [MARTINGALE ERROR] Incomplete results (Expected 1 result). Resetting stake to base.")
        total_profit = 0
    else:
    ...
    if total_profit < 0:
        ...
        # 🛑 منطق المضاعفة: نستخدم الاتجاه والحاجز المحفوظين من الصفقة الخاسرة السابقة (نفس الاتجاه)
        # لا تغيير هنا
        
        print(f"🔄 [LOSS] PnL: {total_profit:.2f}. Step {current_data['current_step']}. Next Stake calculated: {round(new_stake, 2):.2f}. **Ready for IMMEDIATE ENTRY (Same Direction: {current_data['last_entry_direction']}).**")
    ...
    # ✅ Win or Draw Condition (ربح أو تعادل)
        ...
        # 🛑 عند الربح/التعادل: ننتظر الثانية 0 ونطلب تحليل جديد
        ...
        entry_tag = f"WAITING @ SEC {ALLOWED_ENTRY_SECONDS} (For Analysis)" 
        print(f"✅ [ENTRY RESULT] {entry_result_tag}. PnL: {total_profit:.2f}. Stake reset to base: {base_stake_used:.2f}. **Ready for Sec {ALLOWED_ENTRY_SECONDS}.**")
    ...
    print(f"[LOG {email}] PNL: {currency} {current_data['current_profit']:.2f}, Step: {current_data['current_step']}, Stake: {current_data['current_stake_lower']:.2f}, Strategy: 30-TICK Reversed Base | Martingale: Same Direction | Next Entry: {entry_tag}")
...
def handle_contract_settlement(email, contract_id, profit_loss):
    ...
    # بما أن الصفقة فردية، نبدأ منطق المضاعفة بمجرد بيعها
...
def start_new_single_trade(email, analysis_entry_type=None):
    """ 
    يرسل صفقة واحدة: يعكس اتجاه تحليل الـ 30 تيكات في الخطوة 0، ويستخدم الاتجاه الأخير (نفس الاتجاه) في الخطوات 1+
    """
    ...
    # 1. تحديد الاتجاه والحاجز
    
    if current_step == 0:
        # وضع الأساس (البداية/بعد الربح): يعكس اتجاه التحليل (كما في طلبك)
        ...
        entry_type_tag = f"BASE ENTRY (30-TICK Reversed Analysis: Analysed {analysis_entry_type})"
        
    else:
        # وضع المضاعفة: يستخدم نفس الاتجاه والحاجز المحفوظين من الصفقة الخاسرة السابقة (نفس الاتجاه)
    ...
    print(f"🧠 [SINGLE ENTRY - {entry_timing_tag}] {entry_type_tag} | Direction: {entry_type} | Stake: {round(stake, 2):.2f}. Barrier: {barrier_to_use}")
    
    # إرسال صفقة واحدة
    if send_trade_order(email, stake, currency_to_use, entry_type, barrier_to_use):
        is_contract_open[email] = True # حظر الدخول حتى يتم البيع
    else:
        # إذا فشل الإرسال، نزيل الحظر وننتظر الدخول التالي
        is_contract_open[email] = False
        if current_step == 0: LAST_ENTRY_SECOND[email] = None # ننتظر الثانية 0 مجدداً في حال الفشل بالـ Base
...
def bot_core_logic(email, token, stake, tp, currency, account_type):
    ...
    global is_contract_open, active_ws, ALLOWED_ENTRY_SECONDS, LAST_ENTRY_SECOND, TICK_HISTORY_REQUEST_ID, TICK_COUNT_FOR_ANALYSIS
    ...
    while True:
        ...
        def on_message_wrapper(ws_app, message):
            ...
            if msg_type == 'tick':
                ...
                # استخدام datetime لمعرفة الثانية
                ...
                # === منطق طلب التاريخ/الدخول ===
                if not is_contract_open.get(email):
                    
                    current_step = current_data.get('current_step', 0)
                    should_request_history = False
                    should_enter_now_martingale = False
                    
                    if current_step == 0:
                        # 1. وضع عادي (بداية/ربح): ننتظر الثانية 0 ونطلب التحليل
                    ...
                    if should_request_history:
                        LAST_ENTRY_SECOND[email] = current_second # وضع علامة التوقيت لمنع الدخول المتعدد في نفس الثانية
                        entry_tag = f"@ SEC {current_second} (Base)"
                        
                        print(f"⌛ [ENTRY {entry_tag}] Requesting last {TICK_COUNT_FOR_ANALYSIS} Ticks history for analysis...")

                        # 🌟 إرسال طلب جلب آخر 30 تيك
                        request_history = {
                            "ticks_history": SYMBOL,
                            "count": TICK_COUNT_FOR_ANALYSIS,
                            "end": "latest",
                            "style": "ticks",
                            "req_id": TICK_HISTORY_REQUEST_ID 
                        }
                ...
                # 🌟 معالجة استجابة التاريخ (فقط للصفقة الافتتاحية)
                if data.get('req_id') == TICK_HISTORY_REQUEST_ID:
                    
                    current_step = current_data.get('current_step', 0)
                    if current_step != 0:
                        # نتجاهل الـ history إذا كنا في خطوة مضاعفة
                        return
                        
                    if 'history' not in data or 'prices' not in data['history']:
                        print("❌ [HISTORY ERROR] Prices data missing in history response. Skipping trade.")
                        is_contract_open[email] = False 
                        LAST_ENTRY_SECOND[email] = None 
                        return
                    ...
                    if analysis_direction:
                        ...
                        is_contract_open[email] = False 
                        LAST_ENTRY_SECOND[email] = None # ننتظر الثانية 0 مجدداً
                ...
                print(f"❌❌ [API ERROR] Code: {error_code}, Message: {error_message}. **Trade disrupted.**")
...
{% if session_data and session_data.is_running %}
    {% set strategy = "30-TICK Reversed Base Entry | Same Direction Martingale (x" + martingale_multiplier|string + " - Max Steps " + martingale_steps|string + ")" %}
    ...
    <p style="font-weight: bold; color: #cc6600;">Current Barrier: {{ session_data.last_barrier_value }} | Last Direction: {{ session_data.last_entry_direction }}</p>
...
def start_bot():
    ...
    try:
        ...
        # إذا كنت تشغل بوت جديد، قم بمسح البيانات القديمة تمامًا
        delete_session_data(email)
    ...
    flash(f'Bot started successfully. Currency: {currency}. Account: {account_type.upper()}. Strategy: 30-TICK Reversed Base Entry ({SYMBOL} - Entry @ Sec {ALLOWED_ENTRY_SECONDS}) with Same Direction Martingale (x{MARTINGALE_MULTIPLIER} - Max {MARTINGALE_STEPS} Steps, Max {MAX_CONSECUTIVE_LOSSES} Losses)', 'success')
    return redirect(url_for('index'))
