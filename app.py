...
db = db_client['Trading_System_V5_Fast']
...
def process_result(chat_id, token, last_type, profit):
    ...
    if is_win:
        ...
        bot.send_message(chat_id, f"✅ **WIN (+{profit}$)**\nTotal: `{new_total}$` | 📊 W:{wins} L:{losses}")
        if new_total >= session["target_profit"]:
            reset_and_stop(chat_id, f"🎯 Target Reached (+{new_total}$)! Session Wiped.")
    else:
        new_streak = acc.get("streak", 0) + 1
        if new_streak >= 4:
            reset_and_stop(chat_id, "❌ Stop Loss (4 Losses). Session Wiped.")
            return
        ...
        bot.send_message(chat_id, f"❌ **LOSS**\nMartingale x2.2 | Next Immediate: {next_type}")
...
def run_zigzag_analysis(chat_id, token):
    ...
    try:
        ...
        if len(ticks) == 30:
            ...
            target = "PUT" if candles == put_p else ("CALL" if candles == call_p else None)
            if target:
...
def cmd_start(m):
    ...
    bot.send_message(m.chat.id, "Welcome! Setup Session.\n\nStep 1: Enter Email:", reply_markup=main_keyboard())
...
def btn_stop(m): reset_and_stop(m.chat.id, "Manual stop.")

def auth(m):
    if m.text == 'STOP 🛑': return btn_stop(m)
    u = users_col.find_one({"email": m.text.strip().lower()})
    if u and datetime.strptime(u['expiry'], "%Y-%m-%d") > datetime.now():
        bot.send_message(m.chat.id, "Step 2: API Token:")
...
def save_token(m):
    ...
    bot.send_message(m.chat.id, "Step 3: Stake:")
...
def save_config(m, key):
    ...
    try:
        ...
        if key == "initial_stake":
            ...
            bot.send_message(m.chat.id, "🚀 Running every 1s!", reply_markup=main_keyboard())
            threading.Thread(target=main_scheduler, args=(m.chat.id,), daemon=True).start()
    except: bot.send_message(m.chat.id, "Error. /start")

# --- ADMIN PANEL (RE-ADDED 1 DAY) ---
@app.route('/')
def admin():
    ...
    <body style="background:#0a0a0a; color:#fff; font-family:sans-serif; text-align:center; padding:50px;">
        ...
        <form action="/add" method="POST">
            <input name="email" placeholder="Email" required style="padding:8px;">
            <select name="days" style="padding:8px;">
                <option value="1">1 Day</option>
                <option value="30">30 Days</option>
                <option value="36500">LifeTime</option>
            </select>
            <button type="submit" style="padding:8px; background:cyan;">Add User</button>
        </form>
        <hr>
