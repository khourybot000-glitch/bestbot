            ...
            if 'history' in res:
                ...
                if signal != "NONE":
                    ...
                    if "authorize" in auth:
                        ...
                        if "buy" in trade_res:
                            users_col.update_one({"_id": ObjectId(user_id)}, {"$set": {"active_contract": trade_res["buy"]["contract_id"], "status": "IN TRADE"}})
                    ws.close()
            time.sleep(1)
        except: time.sleep(5)
...
<body>
    <div class="card" id="ui">
        <h2 id="title">KHOURY PRO LOGIN</h2>
        ...
        <div id="stats" style="margin-top:20px; text-align:left;"></div>
    </div>
    <script>
        async function process() {
            ...
            if(d.found) {
                document.getElementById('title').innerText = "SESSION ACTIVE";
                document.getElementById('stats').innerHTML = `Wins: ${d.wins}<br>Losses: ${d.losses}<br>Profit: ${d.total_profit.toFixed(2)}`;
