    ...
    try:
        ...
        if prop:
            ...
            if "buy" in buy_res:
                ...
                bot.send_message(state_proxy["chat_id"], f"🎯 **Trade Opened: UNDER 8**\n💰 Stake: {state_proxy['current_stake']}")
                ws.close() # قطع الاتصال فوراً كما طلبت
                return True
        ws.close()
    except: pass
    return False
...
def check_digit_result(state_proxy):
    # انتظار 6 ثوانٍ قبل التحقق من النتيجة
    ...
    try:
        ...
        if contract.get("is_expired") == 1:
            ...
            if is_win:
                ...
                # التوقف بعد 3 خسارات متتالية
                if state_proxy["consecutive_losses"] >= 3:
                    reset_and_stop(state_proxy, "Stopped after 3 losses.")
                    return
                else:
                    state_proxy["current_stake"] = state_proxy["initial_stake"] * 5 # مضاعفة ×5
...
def execute_digit_logic(state_proxy):
    ...
    try:
        ...
        if len(prices) >= 2:
            t1 = float(prices[0])
            t2 = float(prices[1]) # الأحدث
            
            # استخراج الرقم الثاني بعد الفاصلة لـ T2
            t2_str = f"{t2:.2f}" 
            parts = t2_str.split('.')
            if len(parts) > 1 and len(parts[1]) >= 2:
                second_decimal = parts[1][1]
                
                # شرط التحليل: الرقم الثاني بعد الفاصلة هو 7 و T2 أصغر من T1
                if second_decimal == "7" and t2 < t1:
...
def main_loop(state_proxy):
    while True:
        try:
            ...
            time.sleep(0.3) # فحص مستمر وسريع لكل تيك
        except: time.sleep(1)

@bot.message_handler(commands=['start'])
def welcome(m):
    state["chat_id"] = m.chat.id
    bot.send_message(m.chat.id, "🤖 **Digit Under 8 Bot v5.0**\n- Token Updated\n- Rule: T2[2nd Dec] == 7 & T2 < T1\n- Martingale: x5 | Stop on 3 Losses", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add('Demo 🛠️', 'Live 💰'))
...
def save_tp(m):
    ...
    bot.send_message(m.chat.id, "🚀 Running Digit Strategy...", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add('STOP 🛑'))
