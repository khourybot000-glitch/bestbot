...
app = Flask(__name__)
@app.route('/')
def health_check(): return "Professional 20-Indicator Bot Active", 200
...
def analyze_20_indicators(df):
    ...
    # 1-3: RSI (3 periods)
    rsi7 = ta.rsi(c, length=7).iloc[-1]
    rsi14 = ta.rsi(c, length=14).iloc[-1]
    rsi21 = ta.rsi(c, length=21).iloc[-1]
    for r in [rsi7, rsi14, rsi21]:
        if r < 50: up += 1
        else: down += 1

    # 4-6: Moving Averages (EMA 8, 21, 50)
    ema8 = ta.ema(c, length=8).iloc[-1]
    ema21 = ta.ema(c, length=21).iloc[-1]
    ema50 = ta.ema(c, length=50).iloc[-1]
    for e in [ema8, ema21, ema50]:
        if c.iloc[-1] > e: up += 1
        else: down += 1
    ...
    if c.iloc[-1] < bb.iloc[1]: up += 1 # تحت خط الوسط
    else: down += 1
    if (bb.iloc[2] - bb.iloc[0]) > (bb.iloc[2] * 0.001): up += 1 # سيولة صاعدة
    else: down += 1
    ...
    # 15-16: ADX (Trend Strength)
    adx = ta.adx(h, l, c).iloc[-1]
    if adx.iloc[1] > adx.iloc[2]: up += 1 # DI+ > DI-
    else: down += 1
    if adx.iloc[0] > 20: up += 1 # ترند قوي
    else: down += 1

    # 17-18: Price Action (Candles)
    if c.iloc[-1] > o.iloc[-1]: up += 1 # شمعة خضراء
    else: down += 1
    if c.iloc[-1] > c.iloc[-2]: up += 1 # أعلى من إغلاق سابق
    else: down += 1

    # 19-20: MFI & Ichimoku (Simplified)
    ...
    if up > down:
        direction = "CALL (BUY) 🟢"
        accuracy = (up / total_indicators) * 100
    elif down > up:
        direction = "PUT (SELL) 🔴"
        accuracy = (down / total_indicators) * 100
    else:
        direction = "NEUTRAL ⚪"
        accuracy = 50.0
...
def run_analysis():
    ...
    is_bot_busy = True
    active_trade = {"exit_time": entry_time + timedelta(minutes=5)}

    msg = f"🛡 **PREMIUM SIGNAL (20 IND)**\n"
    msg += f"🌍 Pair: EURGBP (5M)\n"
...
def start_engine():
    ...
    print("20-Indicator Engine Active...")
    while True:
        now = datetime.now(BEIRUT_TZ)
        # العمل من الاثنين للجمعة، 9ص - 9م
