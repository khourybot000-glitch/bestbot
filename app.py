...
# --- CONFIGURATION ---
TOKEN = "8433565422:AAHY4TMoAbu1TPWcu16_DKKs_91jZEmLsvE"
        ...
        if now.second in [0, 20, 40]:
            ...
            if current_mark != last_trigger_time:
                ...
                if res and "history" in res:
                    ...
                    # --- NEW: SEND TICK DATA TO TELEGRAM EVERY ANALYSIS ---
                    bot.send_message(chat_id, f"🔍 *Analysis at {now.second}s:*\nPrice T1: `{t1}`\nPrice T5: `{t5}`\nPrice T10: `{t10}`", parse_mode="Markdown")
                    ...
                    if direction:
                        ...
                        if prop_res and "proposal" in prop_res:
                            ...
                            if exec_res and "buy" in exec_res:
                                ...
                                err = exec_res.get("error", {}).get("message", "Execution Failed")
                                bot.send_message(chat_id, f"❌ *Execution Error:* {err}")
                else:
                    bot.send_message(chat_id, "⚠️ *Data Error:* Failed to fetch ticks from Deriv.")
...
def update_stats(chat_id, token, res):
    ...
    bot.send_message(chat_id, 
        ...
        f"🔄 Next Stake: `{new_stake:.2f}`", parse_mode="Markdown")

    if new_total_net >= session.get("target_profit", 99999) or new_mg >= 2:
        bot.send_message(chat_id, "🛑 *Bot Stopped:* Target reached or max losses met.")
...
def index():
    ...
    <style>
        ...
        h2 { color: #1a73e8; }
        .form-box { background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px; }
...
def start_cmd(m):
    bot.send_message(m.chat.id, "🤖 *Welcome to Pulse Bot*\nEnter Email to verify:")
    bot.register_next_step_handler(m, verify_auth)

def verify_auth(m):
    u = users_col.find_one({"email": m.text.strip().lower()})
    if u and datetime.strptime(u['expiry'], "%Y-%m-%d") > datetime.now():
        bot.send_message(m.chat.id, "✅ Verified! Enter your API Token:"); bot.register_next_step_handler(m, setup_bot)
...
def run_trading(m):
    ...
    bot.send_message(m.chat.id, "🚀 Bot Active! Monitoring at 0, 20, 40 seconds.", reply_markup=types.ReplyKeyboardMarkup(resize_keyboard=True).add('STOP 🛑'))
