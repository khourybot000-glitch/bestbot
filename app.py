...
def apply_martingale_logic(email):
    ...
    if total_profit < 0:
        ...
        current_data['last_barrier_value'] = last_barrier_value_used # حفظ قيمة الحاجز السابق
...
def determine_reversed_martingale_entry(email):
    ...
    # 2. تعيين إشارة الحاجز لزيادة هامش الأمان في المضاعفة:
    if new_contract == "CALL":
        # إذا كان العقد الجديد CALL (Higher)، نحتاج حاجز سالب لتحسين هامش الأمان (-0.7)
        new_barrier_sign = "-" 
    else: # new_contract == "PUT"
        # إذا كان العقد الجديد PUT (Lower)، نحتاج حاجز موجب لتحسين هامش الأمان (+0.7)
        new_barrier_sign = "+"
            ...
            if msg_type == 'tick':
                ...
                if not is_contract_open.get(email):
                    ...
                    if current_data['current_step'] > 0:
                        contract_type_param, barrier_sign, barrier_offset_value = determine_reversed_martingale_entry(email)
